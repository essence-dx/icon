...
mod tests {
    ...
    fn test_perfect_hash() {
        ...
        // Perfect hash should find icons (may have collisions with small datasets)
        // In production with 300K+ icons, collisions are extremely rare
        let home_result = index.lookup_exact("home");
        let arrow_result = index.lookup_exact("arrow");

        // At least one should be found
        assert!(
            home_result.is_some() || arrow_result.is_some(),
            "Should find at least one icon"
        );

        // Should not find non-existent icon (unless hash collision)
        // This is a probabilistic data structure, so we just verify it returns something
        let _ = index.lookup_exact("notfound");
    }
}
