...
impl WasmIconSearch {
    ...
    pub fn new(index_data: &[u8]) -> Result<WasmIconSearch, JsValue> {
        // IconIndex doesn't need deserialization - it's already in the right format
        // Just load it directly from the compressed bytes
        let index = IconIndex {
            fst_bytes: vec![],
            metadata_bytes: index_data.to_vec(),
        };
