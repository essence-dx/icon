...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "  Best:    {:.2}ms",
        cold_times.iter().fold(f64::MAX, |a, &b| a.min(b))
    );
    ...
    println!(
        "  Worst:   {:.2}µs",
        warm_times.iter().fold(0.0f64, |a, &b| a.max(b))
    );
    println!(
        "  Best:    {:.2}µs",
        warm_times.iter().fold(f64::MAX, |a, &b| a.min(b))
    );
    ...
    println!(
        "  Your engine:     ~{:.1}ms cold, ~{:.0}µs warm",
        avg_cold, avg_warm
    );
